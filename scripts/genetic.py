...
def GA_with_elitism_multi_parents(
    population,
    evo_limit,
    fitness_func,
    sol_arr,
    ...
    for i in range(evo_limit):
        ...
        i_end = time.time()
        print(
            f"One iteration of GA lasts: {i_end - i_start} for pop size: {len(population)}"
        )

    return solution


def GA_with_elitism_multi_parents_test(
    population,
    evo_limit,
    fitness_func,
    sol_arr,
...
def run_evolution(
    genome_len, evo_limit, fitness_func, pop_size, time_limit, genetic_type
):
    ...
    if genetic_type == 1:
        solution = GA_with_elitism_multi_parents(
            population,
            evo_limit,
            fitness_func,
            sol_arr,
...
def run_evolution_test(
    genome_len, evo_limit, fitness_func, pop_size, time_limit, genetic_type, s, x, y
):
    population = init_population(genome_len, pop_count=pop_size)
    solution = None
    sol_arr = []
    ...
    # solution = GA_with_2_opt(population, evo_limit, fitness_func, sol_arr, best_val, solution)
    if genetic_type == 1:
        solution = GA_with_elitism_multi_parents_test(
            population,
            evo_limit,
            fitness_func,
            sol_arr,
