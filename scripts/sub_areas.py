    ...
    def generate_paths(self):
        ...
        for i in range(len(end_points)):
            ...
            for k in range(len(parallels)):
                ...
                if (
                    start_point == parallels[k].upper_point
                    or start_point == parallels[k].lower_point
                ):
                    index = k
                    if index != 0:
...
class Areas:
    def __init__(
        self, tracks, clusters, objects, width, plot_print, outer_index
    ) -> None:
        super().__init__()
        ...
        for area in self.areas:
            ...
            for index, paralel in enumerate(area):
                if paralel.upper_group not in upper_groups:
                    ...
                    area_upper_item[upper_groups.index(paralel.upper_group)].append(
                        paralel
                    )
        ...
        for area in self.areas:
            ...
            for index, paralel in enumerate(area):
                if paralel.lower_group not in lower_groups:
                    ...
                    area_lower_item[lower_groups.index(paralel.lower_group)].append(
                        paralel
                    )
    ...
    def is_identical(self, line1, line2):
        if (Point(line1.upper_point).distance(Point(line2.lower_point))) > (
            Point(line1.lower_point).distance(Point(line2.upper_point))
        ):
            point1 = line1.upper_point
            point2 = line2.lower_point
            line = LineString([Point(point1), Point(point2)])
            if (
                line.distance(Point(line1.lower_point)) > 0.00001
                and line.distance(Point(line2.upper_point)) > 0.00001
            ):
                return False
            else:
                return True
            ...
            line = LineString([Point(point1), Point(point2)])
            if (
                line.distance(Point(line1.upper_point)) > 0.00001
                and line.distance(Point(line2.lower_point)) > 0.00001
            ):
                return False
            else:
                return True
    ...
    def equidistant_and_not_identical(self, area):
        ...
        for i in range(len(area) - 1):
            ...
            diff = 0.00001
            if (line.distance(line_next) - 0.5) > diff or self.is_identical(
                area[i], area[i + 1]
            ):
                valid = False
        return valid
            ...
            if right_lines:
                ...
                while not self.equidistant_and_not_identical(area):
                    ...
                    while i < (len(area_copy) - 1):
                        ...
                        diff = 0.00001
                        if (
                            line.distance(line_next) - width
                        ) > diff or self.is_identical(area_copy[i], area_copy[i + 1]):
                            # print('nok, will pop')
                            area_copy.pop(i + 1)
                            i = i - 1
                            if i < 0:
    ...
    def next_point_collision(self, area, objects, plot_print, outer_index):
        ...
        for i in range(len(area) - 1):
            ...
            line_upper = LineString(upper_points.geoms)
            line_lower = LineString(lower_points.geoms)
                ...
                if (
                    ...
                    and (
                        self.center_outside(
                            ...
                            obj_upper,
                            outer_index,
                            obj_upper_index or len(intersection.geoms) > 2,
                        )
                    )
                    and self.distance_not_in_bounds(upper_points)
                ):
            ...
            if ring_lower.intersects(line_lower):
                ...
                if (
                    ...
                    and (
                        self.center_outside(
                            lower_points, obj_lower, outer_index, obj_lower_index
                        )
                        or len(intersection.geoms) > 2
                    )
                    and self.distance_not_in_bounds(lower_points)
                ):
    ...
    def split_by_convex(self, objects, plot_print, outer_index):
        ...
        for area in areas:
            ...
            if area:
                indexes = self.next_point_collision(
                    area, objects, plot_print, outer_index
                )
