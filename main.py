...
from scripts.computational_thread import (
    ClusteringThread,
    ComputationalThread,
    GeneticThread,
    PlotThread,
    VisibilityGraphThread,
)
...
class Window(QWidget):
    ...
    def initUI(self):
        ...
        content.threshInput.textChanged.connect(
            lambda text: self.threshInputCheck(text)
        )
        content.angleInput.textChanged.connect(lambda text: self.angleInputCheck(text))
        content.geneticIterLimit.textChanged.connect(
            lambda text: self.geneticInputChange(text)
        )
        ...
        content.geneticType.cb.currentIndexChanged.connect(
            lambda index: self.geneticTypeChange(index)
        )
    ...
    def advanced_clicked(self):
        ...
        if text == "Advanced options":
            ...
            self.graph_data.set_genetic_limit(
                int(self.contentFrame.geneticIterLimit.text())
            )
        else:
    ...
    def getToPreview(self):
        self.contentFrame.contentStack.setCurrentWidget(self.contentFrame.graphWrapper)
        self.headerFrame.settingsToggle.setCurrentWidget(
            self.headerFrame.backToLoaderWidget
        )

    def backToLoader(self):
        self.contentFrame.contentStack.setCurrentWidget(
            self.contentFrame.stopSimulationWidget
        )
        self.headerFrame.settingsToggle.setCurrentWidget(
            self.headerFrame.backButtonWidget
        )
    ...
    def backtograph(self):
        # print('Helo there')
        self.headerFrame.settingsToggle.setCurrentWidget(
            self.headerFrame.settingsButtons
        )
        self.contentFrame.contentStack.setCurrentWidget(self.contentFrame.graphWrapper)
                        ...
                        if i == 0:
                            ...
                            if linering.is_ccw:
                                line = line.parallel_offset(
                                    self.graph_data.width / 2.1, "left"
                                )
                                line2 = LineString(line.coords)
                                line2 = line2.parallel_offset(
                                    self.graph_data.width / 2.1, "right"
                                )
                            ...
                            line = LineString(
                                [paths_iter[-1], path[i], path[i + 1], path[i + 2]]
                            )
                            linering = LinearRing(
                                [paths_iter[-1], path[i], path[i + 1], path[i + 2]]
                            )
            ...
            for i in range(len(paths_exo) - 1):
                ...
                if linering.is_ccw:
                    ...
                    if len(line1.coords) > 1:
                        ...
                        line2 = line2.parallel_offset(
                            self.graph_data.width / 2.2, "right"
                        )
                        coords = list(line2.coords[::-1])
                    else:
    ...
    def plot_parallels(self, areas):
        ...
        for i in range(len(areas)):
            ...
            for k in range(len(parallels)):
                ...
                self.plot(
                    (parallel.upper_point[0], parallel.lower_point[0]),
                    (parallel.upper_point[1], parallel.lower_point[1]),
                    color,
                    "plot_parallels",
                )
    ...
    def clustering_finished(self, cluster_thread):
        ...
        self.visibility_thread = VisibilityGraphThread(
            cluster_thread.graph, cluster_thread.width, cluster_thread.areas
        )
        self.visibility_thread.start()
        self.visibility_thread.finished.connect(
            lambda: self.visibility_graph_finished(self.visibility_thread)
        )
    ...
    def visibility_graph_finished(self, vis_thread):
        ...
        self.genetic_thread = GeneticThread(
            trd.graph, trd.width, trd.areas, trd.node_graph
        )
        self.genetic_thread.finished.connect(
            lambda: self.genetic_finished(self.genetic_thread)
        )
    ...
    def genetic_finished(self, genetic_thread):
        ...
        clean_plot.finished.connect(
            lambda: self.algorithm_finished(ga.seq, ga.areas, ga.node_graph)
        )
        clean_plot.start()
        self.contentFrame.calculateGA.show()
        # self.stopSimulation()
    ...
    def compute_ga_vis(self):
        self.visibility_thread_a = VisibilityGraphThread(
            self.cl_graph, self.cl_width, self.cl_areas
        )
        self.visibility_thread_a.finished.connect(
            lambda: self.compute_ga_gen(self.visibility_thread_a)
        )
    ...
    def compute_ga_gen(self):
        self.genetic_thread_a = GeneticThread(
            self.ga_graph, self.ga_width, self.ga_areas, self.ga_node_graph
        )
        self.genetic_thread_a.finished.connect(
            lambda: self.genetic_finished(self.genetic_thread_a)
        )
        self.genetic_thread_a.start()
    ...
    def set_complete_file(self):
        ...
        if self.get_graph_data():
            ...
            self.clustering_thread.finished.connect(
                lambda: self.clustering_finished(self.clustering_thread)
            )
            ...
            for j in range(len(areas)):
                for k in range(len(areas[j].node_states)):
                    if node.state == areas[j].node_states[k]:
                        ...
                        if i < len(nodes) - 1:
                            points = self.from_points_to_coords(
                                nodes[i].state, nodes[i + 1].state, node_graph
                            )
                            paths_interstate.append(
    ...
    def plot_upper_second(self, point, color):
        ...
        self.contentFrame.graphWidget2.plot(
            [point[0]],
            [point[1]],
            name="plot_upper_second",
            pen=None,
            symbol="o",
    ...
    def plot_upper(self, point, color):
        # print(f"tady to dojde : {point[0]}")
        # pen = pg.mkPen(color="r")
        self.contentFrame.graphWidget.plot(
            [point[0]],
            [point[1]],
            name="plot_upper",
            pen=None,
